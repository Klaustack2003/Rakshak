...
import { 
  ...
  ArrowRight, MessageSquare, X, Send, LogOut 
} from 'lucide-react';
...
function RakshakBot() {
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      ...
      if (lowerInput.includes('admin')) reply = "Admin access requires authorized credentials.";
      ...
      {isOpen && (
        <div className="bg-slate-900/90 backdrop-blur-xl border border-slate-700 w-80 h-96 rounded-2xl shadow-2xl flex flex-col animate-in slide-in-from-bottom-10 fade-in duration-300">
          ...
          <div className="p-3 border-t border-slate-800 flex gap-2">
            <input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSend()} placeholder="Ask me..." className="flex-1 bg-slate-950 text-white text-xs p-3 rounded-lg outline-none border border-slate-800 focus:border-emerald-500/50" />
            <button onClick={handleSend} className="bg-emerald-500 text-slate-900 p-2 rounded-lg"><Send size={16} /></button>
          </div>
        </div>
      )}
    </div>
    ...
    <div className="min-h-screen bg-slate-950 text-white font-sans selection:bg-emerald-500/30 overflow-x-hidden relative">
      <nav className="fixed w-full z-50 bg-slate-950/80 backdrop-blur-md border-b border-slate-800/50">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <button onClick={onLoginClick} className="bg-white hover:bg-slate-200 text-slate-950 px-6 py-2 rounded-full font-bold transition-all hover:scale-105 shadow-[0_0_20px_rgba(255,255,255,0.2)] text-sm">
            LOGIN
          </button>
        </div>
      </nav>
      <section className="relative pt-40 pb-20 px-6 flex flex-col items-center text-center z-10">
        ...
        <h1 className="text-5xl md:text-8xl font-black tracking-tighter mb-6 bg-gradient-to-b from-white to-slate-500 bg-clip-text text-transparent max-w-5xl mx-auto animate-in fade-in slide-in-from-bottom-8 duration-1000">
          NEXT-GEN TRAFFIC <br/> <span className="text-emerald-500">SAFETY PROTOCOL</span>
        </h1>
        <p className="text-lg md:text-xl text-slate-400 max-w-2xl mx-auto mb-10 leading-relaxed animate-in fade-in slide-in-from-bottom-12 duration-1000 delay-200">
          AI-driven accident detection, instant cloud telemetry, and autonomous emergency response.
        </p>
        <button onClick={onLoginClick} className="px-8 py-4 bg-emerald-500 text-slate-950 rounded-xl font-bold flex items-center gap-2 hover:bg-emerald-400 transition-all shadow-[0_0_30px_rgba(16,185,129,0.3)] animate-in fade-in slide-in-from-bottom-16 duration-1000 delay-300">
      ...
      <footer className="border-t border-slate-800 bg-slate-950 py-12 text-center text-slate-600 text-sm">
        <p>&copy; 2026 RAKSHAK INC. DESIGNED BY SUBHADIP DAS.</p>
      </footer>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 font-sans relative overflow-hidden">
      <div className="bg-slate-900/60 backdrop-blur-xl p-8 rounded-3xl shadow-2xl w-full max-w-sm border border-slate-700 relative z-10 animate-in fade-in zoom-in duration-300">
        ...
        <h2 className="text-2xl font-bold text-center mb-1 text-white">{step === 1 ? "Welcome Back" : "Verify Identity"}</h2>
        {step === 1 && (
          <div className="space-y-4">
             <div className="relative">
                <Mail className="absolute left-4 top-3.5 text-slate-500" size={18} />
                <input type="email" placeholder="name@company.com" className="w-full bg-slate-950 text-white pl-12 pr-4 py-3 rounded-xl border border-slate-800 focus:border-emerald-500 outline-none" value={email} onChange={(e) => setEmail(e.target.value)} />
             </div>
             <button onClick={handleSendCode} disabled={isLoading} className="w-full bg-white hover:bg-slate-200 text-slate-950 font-bold py-3 rounded-xl flex justify-center items-center gap-2">{isLoading ? "Processing..." : <>Continue <ArrowRight size={18} /></>}</button>
          </div>
        )}
        {step === 2 && (
          <div className="space-y-4">
             <div className="flex justify-between gap-2">
                {[1,2,3,4].map((_, i) => (<input key={i} type="text" maxLength="1" className="w-14 h-14 bg-slate-950 border border-slate-800 rounded-xl text-center text-xl font-bold text-white focus:border-emerald-500 outline-none" />))}
             </div>
             <p className="text-center text-[10px] text-slate-500">Use any code (Demo)</p>
             <button onClick={handleVerify} disabled={isLoading} className="w-full bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold py-3 rounded-xl shadow-[0_0_20px_rgba(16,185,129,0.3)]">{isLoading ? "Verifying..." : "Access Terminal"}</button>
          </div>
        )}
      </div>
    </div>
       ...
       <div className="max-w-7xl mx-auto">
         <header className="flex justify-between items-center mb-10 bg-slate-900/50 p-4 rounded-2xl border border-slate-800 backdrop-blur-md sticky top-4 z-50">
            <div className="flex items-center gap-3">
               <div className="bg-emerald-500/20 p-2 rounded-lg text-emerald-500"><Activity /></div>
               <div><h1 className="font-bold text-lg leading-none">COMMAND CENTER</h1><span className="text-[10px] text-slate-500 uppercase tracking-widest">Administrator Access</span></div>
            </div>
            <button onClick={onLogout} className="bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2"><LogOut size={14} /> LOGOUT</button>
         </header>
         <div className="grid md:grid-cols-4 gap-6 mb-8">
            <StatBox label="Total Incidents" value={history.length} icon={<AlertTriangle className="text-yellow-500"/>} />
            <StatBox label="Active Nodes" value="842" icon={<Server className="text-blue-500"/>} />
            <StatBox label="System Status" value="OPTIMAL" icon={<CheckCircle className="text-emerald-500"/>} />
            <StatBox label="Avg Response" value="0.4s" icon={<Zap className="text-purple-500"/>} />
         </div>
         <div className="bg-slate-900 rounded-3xl border border-slate-800 overflow-hidden shadow-2xl">
            <div className="p-6 border-b border-slate-800"><h3 className="font-bold text-slate-300">Global Incident Logs</h3></div>
            <table className="w-full text-left text-sm text-slate-400">
              <thead className="bg-slate-950 text-xs uppercase font-bold text-slate-500">
                <tr><th className="p-4">ID</th><th className="p-4">Location</th><th className="p-4">Message</th><th className="p-4">Time</th></tr>
              </thead>
              <tbody className="divide-y divide-slate-800">
                {history.map((log) => (
                  <tr key={log.id} className="hover:bg-slate-800/50 transition-colors">
                    <td className="p-4 font-mono text-emerald-500">#{log.id}</td>
...
function UserApp({ onLogout }) {
  ...
  // FIXED: Use useRef instead of useState for Audio
  ...
  }, []); // Audio ref is stable, no dep needed, but empty array is fine
        ...
        {activeTab === 'home' && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className={`bg-slate-900/50 backdrop-blur-xl p-6 rounded-3xl border transition-all duration-500 ${autoMode ? 'border-emerald-500/50 shadow-[0_0_40px_rgba(16,185,129,0.1)]' : 'border-slate-800'}`}>
              ...
              {/* FIXED: Deterministic Render for Bars */}
              <div className="h-24 bg-slate-950 rounded-lg flex items-end justify-center gap-1 p-2 border border-slate-800 relative overflow-hidden mb-6">
                {[...Array(20)].map((_, i) => (
                  <div key={i} className={`flex-1 rounded-t-sm transition-all duration-100 ${acceleration > 5 ? 'bg-emerald-500' : 'bg-slate-700'}`} 
                       style={{ height: `${Math.min(100, (acceleration * 5) + ((i % 5) * 5))}%`, opacity: autoMode ? 1 : 0.2 }}></div>
                ))}
              </div>
