...
const Input = ({ placeholder, value, onChange, className }: any) => (
  <input 
    ...
    value={value} 
    onChange={onChange} 
    className={`w-full bg-slate-950 text-white p-3 rounded-lg border border-slate-800 focus:border-cyan-500 outline-none placeholder:text-slate-600 text-xs font-mono ${className}`}
  />
);
        ...
        {role === 'user' && (
            <div className="mt-6 space-y-4">
                ...
                <form onSubmit={(e) => handleAuth(isRegistering, e)} className="space-y-4">
                    ...
                    <button type="submit" disabled={isLoading} className="w-full bg-cyan-700 hover:bg-cyan-600 text-white font-bold py-3 rounded-lg transition-all text-sm flex items-center justify-center">
...
function UserApp({ onLogout, user }: { onLogout: () => void, user: any }) {
  ...
  const triggerSOS = async () => {
    ...
    const googleMapsLink = `http://googleusercontent.com/maps.google.com/?q=${location.lat.toFixed(7)},${location.lng.toFixed(7)}`;
