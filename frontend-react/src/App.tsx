...
// --- VISUAL COMPONENTS (IMPORTED AS REQUESTED) ---
...
// --- CUSTOM UI WRAPPERS (To fix TypeScript errors without changing UI) ---
// Using strict types for events to prevent 'implicitly has any type' errors
const CustomInput = ({ placeholder, value, onChange, className }: { 
  placeholder: string; 
  value: string; 
...
const CustomButton = ({ children, onClick, className, variant = 'primary' }: { 
...
function UserApp({ onLogout, user }: { onLogout: () => void, user: any }) {
  ...
  const triggerSOS = async () => {
    ...
    setIsSOSActive(true);
    setStatusLog("CRASH DETECTED! INITIATING EMERGENCY PROTOCOLS...");
    ...
    // Using standard Google Maps Query Link
  ...
  useEffect(() => {
    const watchId = navigator.geolocation.watchPosition(
      (pos) => {
        ...
        // FIX: Clamp altitude to 0 if negative
    ...
    <div className="min-h-screen bg-black text-gray-200 pb-20 font-sans">
      <header className="bg-slate-900/80 backdrop-blur-md border-b border-slate-800 p-4 sticky top-0 z-40">
        <div className="flex justify-between items-center max-w-lg mx-auto">
          ...
          <CustomButton variant="ghost" className="p-2" onClick={onLogout}><LogOut size={20} /></CustomButton>
        </div>
      </header>
        ...
        {activeTab === 'defense' && (
          <div className="space-y-4">
            <div className="grid grid-cols-3 gap-2">
              <div className="bg-slate-900/50 p-3 rounded-xl border border-slate-800 text-center flex flex-col items-center"><Activity size={14} className="text-cyan-600 mb-1" /><div className="text-[10px] text-slate-500 font-bold tracking-widest">SPEED</div><div className="text-xl font-mono text-cyan-400">{stats.speed.toFixed(1)} <span className="text-[10px]">km/h</span></div></div>
          ...
          <div className="space-y-6">
            <div className="bg-slate-900 p-5 rounded-2xl border border-slate-800 space-y-4">
               ...
               <div className="space-y-2">
                 <CustomInput placeholder="Name" className="bg-slate-950 border-slate-800 text-white" value={newContact.name} onChange={(e) => setNewContact({...newContact, name: e.target.value})}/>
                 <CustomInput placeholder="Phone (Required)" className="bg-slate-950 border-slate-800 text-white" value={newContact.phone} onChange={(e) => setNewContact({...newContact, phone: e.target.value})}/>
                 <CustomInput placeholder="Telegram ID (Required for Auto-Text)" className="bg-slate-950 border-slate-800 text-white" value={newContact.telegramId} onChange={(e) => setNewContact({...newContact, telegramId: e.target.value})}/>
                 <div className="text-[10px] text-slate-500">To get ID: Search for your bot in Telegram, type /start.</div>
                 <CustomButton onClick={handleAddContact} className="w-full bg-cyan-600 hover:bg-cyan-700 text-white font-bold">SAVE CONTACT</CustomButton>
               </div>
            </div>
